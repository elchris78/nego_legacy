...
export const DeleteUserModal = ({ isOpenModal, onCloseModal, id }: Props) => {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      // toast(
      //   <ToastSuccessMsg description="Se ha eliminado el usuario de forma exitosa" />
      // );
      onCloseModal();
      ...
      // toast(
      //   <ToastErrorMsg
      //     description={errorMsg ?? "Ocurrió un error al eliminar el usuario"}
      //   />
      // );
    } finally {
      setIsLoading(false);
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl relative">
          <div className="bg-[#D05559] text-white px-6 py-2 rounded-t-xl flex justify-between items-center">
            <h2 className="text-3xl font-thin">Eliminar usuario</h2>
            <button
          ...
          <div className="flex flex-row px-8 py-4">
            ...
            <div className="flex justify-center items-center text-center">
              <span className="antialiased text-sm font-semibold">
                Al eliminar el usuario, ya no podrá acceder a la plataforma,
                pero todas sus operaciones seguirán registradas en la base de
                datos.
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-center w-full gap-4 mb-4">
            <Button
              variant="outline"
              className="border-[#D05559] text-[#D05559] w-32 hover:text-[#D05559] "
              onClick={onCloseModal}
              disabled={isLoading}
            >
              Cancelar
            ...
            <Button
              ...
              className="px-4 py-2 w-32 bg-[#D05559] text-white rounded-md hover:bg-[#df676b] transition duration-150"
              disabled={isLoading}
            >
              Confirmar
            </Button>
