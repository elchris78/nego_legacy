...
export const ClaimsManager = ({ claims,company }: dataClaims) => {
  ...
  useEffect(() => {
    ...
    console.log("Setting CLaims Perm Form Admn"); // Verificar si están vacíos
    setClaims((prev) => ({
      ...prev,
...
export const PermissionsFormAdmin: React.FC<Props> = ({company,companyName} ) => {
  ...
  return (
    ...
    {loader && <Loading />}
      <CardContent sx={{ p: 0 }}>
        ...
        <ClaimsManager claims={claimsData} company={{ value: company.value, label: company.label }}/>
      </CardContent>
    </>
  )
}
