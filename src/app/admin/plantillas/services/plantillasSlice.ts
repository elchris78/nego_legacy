import { createPlantillasSlice } from "./createSlices";
...
interface PlantillasState {
  ...
  roleTemplatesByCompany: RoleResponse[];
...
const initialState: PlantillasState = {
  ...
  roleTemplatesByCompany: [],
  roleTemplateById: null,
  loading: false,
  error: null,
};

const plantillasSlice = createPlantillasSlice({
  name: "plantillas",
  initialState,
  reducers: (createRx) => ({
    // Reducers originales
    ...
    setRoleTemplatesByCompany: createRx.reducer((state, action: PayloadAction<RoleResponse[]>) => {
      state.roleTemplatesByCompany = action.payload;
    }),
    ...
    getTemplates: createRx.asyncThunk(
      async (
        { token, params }: { token: string; params?: RolTemplatesAdminParams },
        thunkAPI
      ) => {
    ...
    // Async Thunk para obtener role templates por compañía
    getTemplatesByCompany: createRx.asyncThunk(
      async ({ token, companyId }: { token: string; companyId: string }, thunkAPI) => {
        thunkAPI.dispatch(setLoading(true));
        thunkAPI.dispatch(setError(null));
        try {
          const response: GetRolesByCompanyAndCompanyIdResponse = await apiRoleTemplates.getTemplatesByCompany(token, companyId);
          if (response.success) {
            thunkAPI.dispatch(setRoleTemplatesByCompany(response.roles));
          } else {
            throw new Error(response.message);
          }
          thunkAPI.dispatch(setLoading(false));
  ...
  selectors: {
    ...
    templatesByCompany: (state: PlantillasState) => state.roleTemplatesByCompany,
