...
const DeleteSucursalModal = ({ isOpenModal, onCloseModal, id }: Props) => {
  ...
  const handleDeleteArea = async () => {
    try {
       const resultAction = await dispatch(
         sucursalActions.deleteSucursal({ token, id })
       );
       if (sucursalActions.deleteSucursal.rejected.match(resultAction)) {
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar está Sucursal?
          </span>
        </div>
