...
//import { useSucursalesFiles } from "../../hooks/useSucursalesFiles"; 
...
import { useSucursalFiles } from "../../hooks/useSucursalesFiles";
...
const printClaim =
  "Configuración.Configuración empresa.Información de la empresa.Sucursales.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handlePdf, handleExcel, handlePrint } = useSucursalFiles({
    sucursal,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${sucursal?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
     const resultAction = await dispatch(
       sucursalActions.importSucursalFromExcel({ token, file })
     );

     if (sucursalActions.importSucursalFromExcel.rejected.match(resultAction)) {
  ...
  const getData = async () => {
     await dispatch(
       sucursalActions.getSucursal({ token, params: { page: 1, size: 20 } })
     );
  };
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-sucursales.xlsx"}
          formatFileName={"formato-sucursales.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
