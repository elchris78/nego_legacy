...
export const usePuestosFiles = ({ puestos = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(puestos || []).map((puestos) => [
        puestos.id && puestos.id.includes("-") ? puestos.id.split("-")[0] : "-",
        puestos.id || "-",
        { text: puestos.nombre || "-", color: "#3C98CB" },
        puestos.descripcion,
        puestos.aplicaPara,
      ]),
    ];
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("Puestos.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (puestos && puestos.length > 0) {
      const renamedData = puestos.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Puesto": item.id,
        "Nombre del Puesto": item.nombre,
      ...
      XLSX.writeFile(workbook, "Puestos.xlsx");
