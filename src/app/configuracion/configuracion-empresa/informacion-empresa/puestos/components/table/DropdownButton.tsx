...
import { usePuestosFiles } from "../../hooks/usePuestosFiles";
...
const DropdownButton = () => {
  ...
  const { handlePdf, handleExcel, handlePrint } = usePuestosFiles({
    puestos,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${puestos?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      puestosActions.importPuestos({ token, file })
    );
    if (puestosActions.importPuestos.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      puestosActions.getPuestos({ token, params: { page: 1, size: 20 } })
    );
  };
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {hasClaim(
            "Configuración.Configuración empresa.Información de la empresa.Puestos.Imprimir"
          ) && (
            <DropdownMenuItem
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-Puestos.xlsx"}
          formatFileName={"formato-Puestos.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
