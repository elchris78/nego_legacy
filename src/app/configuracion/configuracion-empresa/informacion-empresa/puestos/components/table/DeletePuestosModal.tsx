...
import { deletePuestos } from "../../services/puestosAction";
...
const DeletePuestosModal = ({ isOpenModal, onCloseModal, id,getPuesto }: Props) => {
  ...
  const handleDeletePuesto = async () => {
    setIsLoading(true);
    try {
      const resp = await deletePuestos({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  resp?.message || "El puesto ha sido eliminada correctamente.",
        icon: "success",
      ...
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar el puesto. Por favor, inténtalo de nuevo.",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar el Puesto de forma definitiva?
          </span>
        </div>
...
export default DeletePuestosModal;
