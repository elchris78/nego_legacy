...
const EmpresaDocumentacionAdicionalTableRow = ({
  index,
  startIndex,
  documento,
  getData = () => {},
  ...
  const isPending = useSelector(
    (state: RootState) => state.empresaDocumentacion.isPending
  );
  const currentDocument = useSelector(
    (state: RootState) => state.empresaDocumentacion.currentDocumentacion
  );
  const isLoadingDocument = useSelector(
    (state: RootState) => state.empresaDocumentacion.isLoadingDocument
  );
  ...
  const handleDeleteItem = async () => {
    try {
      const resultAction = await dispatch(
        empresaDocumentacionAdicionalActions.deleteEmpresaDocumentacionAdicional(
          {
            token,
            id: documento?.id,
            empresaId: empresaId!,
      ...
      if (
        empresaDocumentacionAdicionalActions.deleteEmpresaDocumentacionAdicional.rejected.match(
          resultAction
        )
      ) {
  ...
  const handleGetDocument = async () => {
    try {
      const resultAction = await dispatch(
        empresaDocumentacionAdicionalActions.getEmpresaDocumentacionAdicionalByID(
          {
            token,
            id: documento?.id,
            empresaId: empresaId!,
      ...
      if (
        empresaDocumentacionAdicionalActions.getEmpresaDocumentacionAdicionalByID.rejected.match(
          resultAction
        )
      ) {
        throw resultAction.payload;
  ...
  const handleSubmitFile = async (file: File | undefined, name: string) => {
    try {
      const resultAction = await dispatch(
        empresaDocumentacionAdicionalActions.updateEmpresaDocumentacionAdicional(
          {
            token,
            id: documento?.id,
            body: {
      ...
      if (
        empresaDocumentacionAdicionalActions.updateEmpresaDocumentacionAdicional.rejected.match(
          resultAction
        )
      ) {
        throw resultAction.payload;
...
export default EmpresaDocumentacionAdicionalTableRow;
