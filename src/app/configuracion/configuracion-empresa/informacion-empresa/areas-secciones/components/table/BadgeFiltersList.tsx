...
const BadgeFiltersList = ({
  ...
  const hasFiltersApplied =
    ...
    searchParams.responsibleId && searchParams.responsibleId.length > 0;
  ...
  return (
    <Stack direction="row" flexWrap="wrap" gap={1} spacing={1} mt={2}>
      ...
      {searchParams.responsibleId &&
        searchParams.responsibleId.map((responsable) => {
          const responsableOption = responsableOptions.find(
            (opt) => opt.value === responsable
          );
          return (
            <Chip
              key={responsable}
              label={responsableOption?.label}
              onDelete={() => removeFilter(responsable, "responsibleId")}
            />
          );
        })}
    </Stack>
