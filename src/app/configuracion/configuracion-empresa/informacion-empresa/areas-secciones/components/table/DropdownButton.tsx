...
import { useAreasFiles } from "../../hooks/useAreasFiles";
...
const printClaim =
  "Configuración.Configuración empresa.Información de la empresa.Áreas / Secciones.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handlePdf, handleExcel, handlePrint } = useAreasFiles({
    areas,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${areas?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      areasActions.importAreas({ token, file })
    );

    if (areasActions.importAreas.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      areasActions.getAreas({ token, params: { page: 1, size: 20 } })
    );
  };
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-áreas-secciones.xlsx"}
          formatFileName={"formato-áreas-secciones.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
