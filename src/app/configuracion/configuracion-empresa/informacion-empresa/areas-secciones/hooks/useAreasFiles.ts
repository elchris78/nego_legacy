...
export const useAreasFiles = ({ areas = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(areas || []).map((area) => [
        area.id && area.id.includes("-") ? area.id.split("-")[0] : "-",
        area.id,
        { text: area.nombre || "-", color: "#3C98CB" },
        area.nombreResponsable,
        area.descripcion,
      ]),
    ];
    return {
      ...
      content: [
        { text: "Áreas/Secciones", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("áreas-secciones.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (areas && areas.length > 0) {
      const renamedData = areas.map((item) => ({
        UID: item.uidArea,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave de área": item.id,
        Nombre: item.nombre,
        Descripción: item.descripcion,
        Estatus: item.estatus ? "Activo" : "Inactivo",
        Responsable: item.nombreResponsable,
      }));

      const worksheet = XLSX.utils.json_to_sheet(renamedData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Áreas Secciones");

      XLSX.writeFile(workbook, "areas-secciones.xlsx");
