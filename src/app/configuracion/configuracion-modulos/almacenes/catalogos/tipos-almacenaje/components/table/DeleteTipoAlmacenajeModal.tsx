...
import { deleteTiposAlmacenaje } from "../../services/tipoAlmacenajeAction";
...
const DeleteTipoAlmacenajeModal = ({ isOpenModal, onCloseModal, id,getTipoAlmacenaje }: Props) => {
  ...
  const handleDeleteEstatusProd = async () => {
    setIsLoading(true);
    try {
      const resp = await deleteTiposAlmacenaje({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar este tipo de almacenaje?
          </span>
        </div>
...
export default DeleteTipoAlmacenajeModal;
