...
const DropdownButton = () => {
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${tipoAlmacenaje?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      tipoAlmacenajeActions.importTipoAlmacenaje({ token, file })
    );
    if (tipoAlmacenajeActions.importTipoAlmacenaje.rejected.match(resultAction)) {
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {hasClaim(
            "Configuración.Configuración de módulos.Almacenes.Catálogos.Tipos de almacenaje.Imprimir"
          ) && (
            <DropdownMenuItem
              className="font-semibold cursor-pointer hover:!bg-[#4197CB42] rounded-sm"
              onClick={handlePrint}
              disabled={tipoAlmacenaje?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/Formato-TiposAlmacenaje.xlsx"}
          formatFileName={"Formato-TiposAlmacenaje.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
