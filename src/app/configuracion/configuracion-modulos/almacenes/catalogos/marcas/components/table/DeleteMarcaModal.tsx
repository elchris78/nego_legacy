...
import { deleteMarcas } from "../../services/MarcaAction"; 
...
const DeletePresentacionesModal = ({ isOpenModal, onCloseModal, id,getMarcas }: Props) => {
  ...
  const handleDeletePresentaciones = async () => {
    setIsLoading(true);
    try {
      const resp = await deleteMarcas({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  
      ...
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar la marca. Por favor, inténtalo de nuevo.",
        icon: "error",
