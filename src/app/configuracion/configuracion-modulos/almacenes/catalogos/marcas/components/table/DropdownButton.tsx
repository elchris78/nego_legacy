...
import { useMarcasFiles } from "../../hooks/useMarcasFiles";
...
const DropdownButton = () => {
  ...
  //   const estatusProd = useSelector((state: RootState) => state.estatusProd.estatusProd); descomentar cuando exista el redux y eliminar el de abajo
  ...
  const { handlePdf, handleExcel, handlePrint } = useMarcasFiles({
    marcas,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${marcas?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      MarcasActions.importMarcas({ token, file }) //cambiar cuando de cree la funcion
    );
    if (MarcasActions.importMarcas.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      MarcasActions.getMarcas({ token, params: { page: 1, size: 20 } })
    );
  };
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {hasClaim(
            "Configuración.Configuración de módulos.Almacenes.Catálogos.Presentaciones.Imprimir"
          ) && (
            <DropdownMenuItem
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/Formato-Marcas.xlsx"}
          formatFileName={"Formato-Marcas.xlsx"}
