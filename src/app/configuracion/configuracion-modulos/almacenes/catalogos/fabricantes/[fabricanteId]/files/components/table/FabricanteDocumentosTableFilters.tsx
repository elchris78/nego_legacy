...
const FabricanteDocumentosTableFilters = ({
  searchParams,
  setSearchParams,
  getData = () => {}, // Optional function to refresh data
}: Props) => {
  ...
  const handleSubmitFile = async (file: File | undefined, name: string) => {
    try {
      const resultAction = await dispatch(
        fabricanteDocumentosActions.createFabricanteDocumento({
          token,
          body: {
            archivo: file,
            nombre: name,
      ...
      if (
        fabricanteDocumentosActions.createFabricanteDocumento.rejected.match(
          resultAction
        )
      ) {
      ...
      showAlert({
        success: true,
        message:
          resultAction.payload.message || "Documento subido correctamente.",
      });
    } catch (error: any) {
...
export default FabricanteDocumentosTableFilters;
