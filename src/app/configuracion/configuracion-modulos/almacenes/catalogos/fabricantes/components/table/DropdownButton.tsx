...
import { useFabricantesFiles } from "../../hooks/useFabricantesFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Almacenes.Catálogos.Fabricantes.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useFabricantesFiles({
    fabricantes,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${fabricantes?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      fabricanteActions.importFabricantes({
        token,
        file,
      })
    );
    if (fabricanteActions.importFabricantes.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      fabricanteActions.getFabricantes({ token, params: { page: 1, size: 20 } })
    );
  };
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-fabricantes.xlsx"}
          formatFileName={"formato-fabricantes.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
