import { CircleEllipsisIcon } from "lucide-react";

import {
  Table,
  TableBody,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import FabricantesTableRow from "./FabricantesTableRow";
import TableLoader from "@/components/ui/Tables/TableLoader";
import EmptyTable from "@/components/ui/Tables/EmptyTable";

import type { Fabricante } from "../../services/fabricantesTypes";

interface Props {
  fabricantes: Fabricante[] | null;
  startIndex: number;
  isLoading?: boolean;
}

const tableHeaders = ["#", "Clave", "Nombre del fabricante", "País", "Estatus"];

const FabricantesTable = ({ fabricantes, startIndex, isLoading }: Props) => {
  return (
    <div className="p-5">
      <Table className="border-[5px] border-solid border-[#EDEDED]">
        <TableHeader className="bg-[#EDEDED]">
          <TableRow className="p-5">
            {tableHeaders.map((header) => (
              <TableHead key={header}>{header}</TableHead>
            ))}
            <TableHead className="flex justify-center items-center">
              <CircleEllipsisIcon />
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {isLoading ? (
            <TableLoader length={tableHeaders.length + 1} />
          ) : fabricantes && fabricantes.length > 0 ? (
            fabricantes.map((fabricante, index) => (
              <FabricantesTableRow
                key={fabricante.id}
                index={index}
                fabricante={fabricante}
                startIndex={startIndex}
              />
            ))
          ) : (
            <EmptyTable length={tableHeaders.length + 1} />
          )}
        </TableBody>
      </Table>
    </div>
  );
};

export default FabricantesTable;
