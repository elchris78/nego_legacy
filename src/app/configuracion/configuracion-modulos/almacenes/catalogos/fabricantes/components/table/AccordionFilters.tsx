...
import type { FabricanteTypeParams } from "../../services/fabricantesTypes";
import MultipleSelector, { Option } from "@/components/ui/multiselect";

interface Props {
  searchParams: FabricanteTypeParams;
  setSearchParams: Dispatch<SetStateAction<FabricanteTypeParams>>;
  onClosePopover: () => void;
...
const AccordionFilters = ({
  ...
  useEffect(() => {
    // Si no hay países seleccionados, se asigna un array vacío
    // Colocar el label compuesto por el valor y el nombre del país
    const selectedOptions = searchParams.paisClave?.map((value) => {
  ...
  }, [searchParams.paisClave, countriesOptions]);

  const handleCheckboxChange = (
    e: React.ChangeEvent<HTMLInputElement>,
    value: string,
    field: keyof Pick<FabricanteTypeParams, "isActive" | "paisClave">
  ) => {
    ...
    <div>
      ...
      <Accordion type="multiple" className="px-3 py-1">
        ...
        <AccordionItem value="countries">
          ...
          <AccordionContent className="px-3 py-2 overflow-visible h-[37vh]">
            <div className="relative overflow-visible">
              <MultipleSelector
                ...
                onChange={(values) => {
                  ...
                  setSearchParams((prevState) => ({
                    ...prevState,
                    paisClave: values.map((item) => item.value),
                  }));
                }}
                emptyIndicator={
