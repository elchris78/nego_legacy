...
import { deleteTypesWarehouses } from "../../services/typesWarehousesAction";
...
const DeleteTypesWarehousesModal = ({ isOpenModal, onCloseModal, id,getTypesWarehouses }: Props) => {
  const token = Cookies.get("auth-token") || "";
  const [isLoading, setIsLoading] = useState(false);

  const handleDeleteEstatusProd = async () => {
    setIsLoading(true);
    try {
      const resp = await deleteTypesWarehouses({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  
        resp?.message || 
        "El estatusP ha sido eliminada correctamente.",
        icon: "success",
      ...
      onCloseModal();
      await getTypesWarehouses()
    } catch (error) {
      console.error("Error al eliminar el estatusP:", error);
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar el estatusP. Por favor, inténtalo de nuevo.",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar este tipo de almacén?
          </span>
        </div>
        ...
        <div className="flex justify-center gap-8 mt-4">
          ...
          <Button
            className="px-8"
            onClick={handleDeleteEstatusProd}
            disabled={isLoading}
          >
            Eliminar
          </Button>
...
export default DeleteTypesWarehousesModal;
