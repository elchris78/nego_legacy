...
import { useEstatusProdFiles } from "../../hooks/useEstatusProdFiles";
...
const DropdownButton = () => {
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${estatusProd?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      estatusProdActions.importEstatusProd({ token, file })
    );
    if (estatusProdActions.importEstatusProd.rejected.match(resultAction)) {
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {hasClaim(
            "Configuración.Configuración de módulos.Almacenes.Catálogos.Estatus producto.Imprimir"
          ) && (
            <DropdownMenuItem
              className="font-semibold cursor-pointer hover:!bg-[#4197CB42] rounded-sm"
              onClick={handlePrint}
              disabled={estatusProd?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/Formato-EstatusProductos.xlsx"}
          formatFileName={"Formato-EstatusProductos.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
