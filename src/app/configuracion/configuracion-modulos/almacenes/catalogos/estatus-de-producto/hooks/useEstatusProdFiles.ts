...
export const useEstatusProdFiles = ({ estatusProd = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    const tableHeaders = [
      "Prefijo",
      "Clave Estatus producto",
      "Nombre del estatus del producto",
      "Estatus",
    ];
    const tableBody = [
      tableHeaders,
      ...(estatusProd || []).map((estatusProd) => [
        estatusProd.id && estatusProd.id.includes("-")
          ? estatusProd.id.split("-")[0]
          : "-",
        estatusProd.id || "-",
        { text: estatusProd.nombre || "-", color: "#3C98CB" },
        estatusProd.estatus ? "Activo" : "Inactivo",
      ]),
    ];
    return {
      pageSize: "A4",
      ...
      content: [
        { text: "Estatus del producto", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("estatus_del_producto.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (estatusProd && estatusProd.length > 0) {
      const renamedData = estatusProd.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Estatus producto": item.id,
        Nombre: item.nombre,
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "estatus_del_producto");

      XLSX.writeFile(workbook, "estatus_del_producto.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Estatus de productos",
    });
  };
