...
import { deleteCategories } from "../../services/categoriesAction"; 
...
const DeleteCategoriesModal = ({ isOpenModal, onCloseModal, id,getCategories }: Props) => {
  ...
  const handleDeletePresentaciones = async () => {
    setIsLoading(true);
    try {
      const resp = await deleteCategories({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  
        resp?.message || 
        "La categories ha sido eliminada correctamente.",
        icon: "success",
      ...
      console.error("Error al eliminar la categories:", error);
      Swal.fire({
        title: "Error",
         text:
      (error instanceof Error && error.message) ||
      String(error) ||
      "No se pudo eliminar la categories. Por favor, inténtalo de nuevo.",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar está categoría de forma definitiva?
          </span>
        </div>
