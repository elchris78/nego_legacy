...
const DropdownButton = () => {
  ...
  const { handlePdf, handleExcel, handlePrint } = useCategoriesFiles({
    categories,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${categories?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      CategoriesActions.importCategories({ token, file })
    );
    if (CategoriesActions.importCategories.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      CategoriesActions.getCategories({ token, params: { page: 1, size: 20 } })
    );
  };
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {hasClaim(
            "Configuración.Configuración de módulos.Almacenes.Catálogos.Categorías.Imprimir"
          ) && (
            <DropdownMenuItem
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-Categorias.xlsx"}
          formatFileName={"formato-Categorias.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
