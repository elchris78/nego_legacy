...
import { getAttributes, importAttributes } from "../../../services/AttributeValueSlice";
...
const UploadFile = ({ onCloseModal }: Props) => {
  ...
  const isLoading = useSelector((state: RootState) => state.values.loading);
  ...
  const submitFile = async (data: FormValues) => {
    ...
    try {
      await dispatch(
        importAttributes({ token, atributoId, file })
      ).unwrap();
      ...
      await dispatch(
        getAttributes({ token, atributoId, requestParams: {page: 1, size:20} })
      );
    }
  };
  ...
  const handleDownloadFormat = async () => {
    ...
    downloadLink.href = "/formatos/formato-valores.xlsx"; // URL del archivo
    downloadLink.download = "Formato-Valores.xlsx";
    downloadLink.click();
  };
