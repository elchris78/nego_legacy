...
const GeneralDataForm = () => {
  ...
  useEffect(() => {
    if ((mode === "edit" || mode === "view") && currentAttribute) {
      setValue("claveValor", currentAttribute.id ?? "");
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
        ...
        <div>
          <LabelTooltip
            label="*Clave Valor"
            tooltip="Identificador único del Valor usado internamente en la empresa"
            htmlFor="id-attributeValue"
          />
          <Input
            id="id-attributeValue"
            type="text"
            placeholder="Ingresa la clave del Valor"
            disabled={
              mode !== "new" ||
            ...
            {...register("claveValor", {
              ...
              validate: (value) => {
                if (
                  keyConfig?.tipoClave !== "Numérico autoincrementable" &&
                  !value
                ) {
                  return "Ingresa la clave de categoría";
                }
        ...
        <div>
          <LabelTooltip
            label="*Estatus"
            tooltip="Selecciona si el Valor está activo o inactivo"
            htmlFor="status-area"
          />
          <Select
            {...register("estatus")}
        ...
        <div>
          <LabelTooltip
            label="*Valor"
            tooltip="Nombre asignado al Valor"
            htmlFor="nombre-valor"
          />
          <Input
            id="nombre-valor"
            type="text"
            placeholder="Ingresa el nombre del Valor"
            disabled={mode === "view"}
            {...register("nombre", {
              required: "El nombre del Valor es requerido",
              pattern: {
              ...
              minLength: {
                value: 1,
                message:
                  "El nombre del Valor debe contener al menos 3 caracteres",
              },
              maxLength: {
                value: 100,
                message:
                  "El nombre del Valor no puede exceder los 100 caracteres",
              },
            })}
          />
          {errors.nombre && (
