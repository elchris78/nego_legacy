...
import { getAttributes, importAttributes } from "../../../services/AttributeSlice";
...
const UploadFile = ({ onCloseModal }: Props) => {
  ...
  const isLoading = useSelector((state: RootState) => state.attribute.loading);
  ...
  try {
    await dispatch(importAttributes({ token, file })).unwrap();
    ...
    await dispatch(
      getAttributes({ token, requestParams: { page: 1, size: 20 } })
    );
  }
};
  ...
  const handleDownloadFormat = async () => {
    ...
    downloadLink.href = "/formatos/formato-atributos.xlsx"; // URL del archivo
    downloadLink.download = "Formato-Atributos.xlsx";
    downloadLink.click();
  }
