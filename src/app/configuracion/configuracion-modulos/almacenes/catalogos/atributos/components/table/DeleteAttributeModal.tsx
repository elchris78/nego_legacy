...
import { deleteAttribute, getAttributeById } from "../../services/AttributeSlice";
import showAlert from "@/lib/utils/alerts";
import { isDayPickerSingle } from "react-day-picker";
import { getAttributes } from "../../[atributoId]/valores/services/AttributeValueSlice";
...
const DeleteAttributeModal = ({
  ...
  const valores = useSelector(
    (state: RootState) => state.values.attributeValuesData
  );
  ...
  useEffect(() => {
    ...
    const fetchAttribute = async () => {
      setIsFetching(true);
      try {
        await dispatch(getAttributes({ token, atributoId:  id  }));
        setIsFetching(false);
      } catch (error) {
    ...
    fetchAttribute();
    console.log("Fetched valores:", valores);
  }, [isOpenModal]);

  // actualizar texto cuando lleguen los atributos
  useEffect(() => {
    if (!isOpenModal) return;
    if (valores.length > 0) {
      setModalText(
        `Este atributo que estás a punto de eliminar contiene valores relacionados. Si confirmas la eliminación, estos también serán eliminados
        ¿Estás seguro que deseas eliminar el atributo de forma definitiva?
      `);
    } else if (valores.length === 0) {
      setModalText(`
        ¿Estás seguro que deseas eliminar el Atributo de forma
        definitiva?
      `);
    } else {
      setModalText('Sin atributos');
    }
  }, [isOpenModal, valores]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showAlert({ success: true, message: "El atributo ha sido eliminado exitosamente" })
      onCloseModal();
    } catch (error) {
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          {isFetching ? (
            ...
            <span className="text-xl font-light">
              {modalText ? modalText : "¿Estás seguro que deseas eliminar el Atributo de forma definitiva?"}
            </span>
            )}
