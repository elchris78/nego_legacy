...
import { getAttributeById } from "../../services/AttributeSlice";
...
interface GeneralDataFormValues {
  claveAtributo: string; // Opcional, si se quiere asignar un ID específico
...
export const AttributesProvider: React.FC<{
  ...
  const { keyConfig, isLoading: isKeyConfigLoading } = useKeyConfigValidation("Atributos"); // Hook para validar la configuración de claves
  ...
  // Redux
  const entity = useSelector((state: RootState) => state.attribute.attribute);
    ...
    await generalDataForm.handleSubmit(
      (data) => {
        ...
        if (
          ...
          body.userProvidedId = claveAtributo?.trim();
        }
  ...
  useEffect(() => {
    const fetchAttribute = async () => {
      if (mode !== "new" && id) {
        try {
          await dispatch(getAttributeById({ token, request: { id } }));
        } catch (error) {
