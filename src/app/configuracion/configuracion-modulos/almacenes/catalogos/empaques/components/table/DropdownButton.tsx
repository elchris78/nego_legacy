...
import { useRestrictionConceptsFiles } from "../../hooks/useEmpaquesFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Almacenes.Catálogos.Empaques.Imprimir";
...
const DropdownButton = () => {
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${empaques?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      empaquesActions.importEmpaques({
        token,
        file,
      })
    );
    if (empaquesActions.importEmpaques.rejected.match(resultAction)) {
  ...
  const getData = async () => {
    await dispatch(
      empaquesActions.getEmpaques({ token, params: { page: 1, size: 20 } })
    );
  };
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-empaques.xlsx"}
          formatFileName={"formato-empaques.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
