...
export const useRestrictionConceptsFiles = ({ empaques = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(empaques || []).map((classification) => [
        classification.id && classification.id.includes("-")
          ? classification.id.split("-")[0]
          : "-",
        classification.id || "-",
        { text: classification.descripcion || "-", color: "#3C98CB" },
        {
          text:
            `${classification.unidadSatId} - ${classification.unidadSat}` ||
            "-",
        },
      ]),
    ];
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("empaques.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (empaques && empaques.length > 0) {
      const renamedData = empaques.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Empaque": item.id,
        Descripción: item.descripcion,
        "Unidad SAT ID": item.unidadSatId,
        Estatus: item.estatus ? "Activo" : "Inactivo",
      }));

      const worksheet = XLSX.utils.json_to_sheet(renamedData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Empaques");

      XLSX.writeFile(workbook, "Empaques.xlsx");
