...
import { deletePresentaciones } from "../../services/presentacionesAction"; 
...
const DeletePresentacionesModal = ({ isOpenModal, onCloseModal, id,getPresentaciones }: Props) => {
  const token = Cookies.get("auth-token") || "";
  const [isLoading, setIsLoading] = useState(false);

  const handleDeletePresentaciones = async () => {
    setIsLoading(true);
    try {
      const resp = await deletePresentaciones({ token, id });
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text:  
        resp?.message || 
        "La prestación ha sido eliminada correctamente.",
        icon: "success",
      ...
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar la presentación. Por favor, inténtalo de nuevo.",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar la Presentación de forma definitiva?
          </span>
        </div>
        ...
        <div className="flex justify-center gap-8 mt-4">
          ...
          <Button
            className="px-8"
            onClick={handleDeletePresentaciones}
            disabled={isLoading}
          >
            Eliminar
          </Button>
...
export default DeletePresentacionesModal;
