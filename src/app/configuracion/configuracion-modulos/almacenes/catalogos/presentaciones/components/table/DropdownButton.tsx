...
import { usePresentacionesFiles } from "../../hooks/usePresentacionesFiles";
...
const DropdownButton = () => {
  ...
  const { handlePdf, handleExcel, handlePrint } = usePresentacionesFiles({
    presentaciones,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${presentaciones?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      presentacionesActions.importPresentaciones({ token, file })
    );
    if (
      presentacionesActions.importPresentaciones.rejected.match(resultAction)
    ) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/Formato-Presentaciones.xlsx"}
          formatFileName={"Formato-Presentaciones.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
