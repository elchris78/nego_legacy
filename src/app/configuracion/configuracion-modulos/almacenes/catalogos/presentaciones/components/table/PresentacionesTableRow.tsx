...
import { togglePresentacionesStatus } from "../../services/presentacionesAction";
import { ComicTooltip } from "@/components/ui/LabelTooltip";
import DeletePresentacionesModal from "./DeletePresentacionesModal";

const toggleStatusClaim =
  "Configuración.Configuración de módulos.Almacenes.Catálogos.Presentaciones.ToggleStatus";
const deleteClaim =
  "Configuración.Configuración de módulos.Almacenes.Catálogos.Presentaciones.Eliminar";
const updateClaim =
...
const PuestosTableRow = ({ index, startIndex, presentaciones, getPresentaciones }: Props) => {
  const token = Cookies.get("auth-token") || "";
  const router = useRouter();

  const [isActive, setisActive] = useState<boolean>(presentaciones?.estatus ?? false);
  const [isOpenModal, setIsOpenModal] = useState(false);

  useEffect(() => {
    setisActive(presentaciones.estatus);
  ...
  const handleToggleStatus = async (newState: boolean) => {
    try {
      ...
      const resp = await togglePresentacionesStatus({ token, id: presentaciones?.id, isActive: !presentaciones?.estatus });
      if (resp?.success) 
        {
        Swal.fire({
          title: "¡ÉXITO!",
          text:
            resp.message ||
            "La prestación actualizado correctamente en la empresa seleccionada.",
          icon: "success",
          confirmButtonText: "Cerrar",
          customClass: {
            container: "swal2-container",
      ...
      Swal.fire({
        title: "¡ERROR!",
        text: "Ocurrió un error al cambiar el estatus de La prestación.",
        icon: "error",
    ...
    <> 
      <TableRow>
        ...
        <TableCell className="text-[#3C98CB]">
          {presentaciones.descripcion?.length > 20 ? (
            <ComicTooltip title={presentaciones?.descripcion} placement="top-start">
              <Link
                className="cursor-pointer hover:underline"
                href={`/configuracion/configuracion-modulos/almacenes/catalogos/presentaciones/form?mode=view&id=${presentaciones?.id}`}
              >
                {presentaciones?.descripcion.slice(0, 20) + "..."}
              </Link>
            </ComicTooltip>
          ):(
          <Link
            ...
            <PopoverContent align="center" className="p-2 w-fit">
              <div className="flex flex-row gap-2">
                { hasClaim(updateClaim) && (
                  <Link
                    href={`/configuracion/configuracion-modulos/almacenes/catalogos/presentaciones/form?mode=edit&id=${presentaciones?.id}`}
