...
import { useSellersFiles } from "../../hooks/useSellersFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Vendedores.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useSellersFiles({
    sellers,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${sellers?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      sellersActions.importSellersFromExcel({
        token,
        file,
      })
    );
    if (
      sellersActions.importSellersFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-vendedor.xlsx"}
          formatFileName={"formato-vendedor.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
