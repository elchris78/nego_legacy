...
export const useClassificationsFiles = ({ classifications = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    return {
      ...
      content: [
        { text: "Clasificación de clientes", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("clasificación-de-clientes.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (classifications && classifications.length > 0) {
      const renamedData = classifications.map((item) => ({
        UID: item.uidClasificacion,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave de clasificación": item.id,
        "Nombre de la clasificación": item.nombre,
      ...
      XLSX.writeFile(workbook, "clasificación-de-clientes.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Clasificación de clientes",
    });
  };
