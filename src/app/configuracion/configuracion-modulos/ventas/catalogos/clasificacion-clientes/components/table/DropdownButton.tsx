...
import { useClassificationsFiles } from "../../hooks/useClassificationsFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Clasificación de clientes.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useClassificationsFiles({
    classifications,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${classifications?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      clientClassificationsActions.importClientClassifications({
        token,
        file,
      })
    );
    if (
      clientClassificationsActions.importClientClassifications.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-clasificacion-clientes.xlsx"}
          formatFileName={"formato-clasificacion-clientes.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
