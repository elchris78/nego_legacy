...
import { deleteClientSubclassification } from "../../services/clientsSubclassificationSlice";
...
const DeleteClientClassificationModal = ({
  isOpenModal,
  onCloseModal,
  id,
}: Props) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showAlert({ success: true, message: "La subclasificación del cliente ha sido eliminada correctamente." })
      onCloseModal();
    } catch (error) {
      showAlert({ success: false, message: `No se pudo eliminar la Subclasificación de Cliente de forma exitosa.` })
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar la Subclasificación de Clientes
            de forma definitiva?
          </span>
        </div>
...
export default DeleteClientClassificationModal;
