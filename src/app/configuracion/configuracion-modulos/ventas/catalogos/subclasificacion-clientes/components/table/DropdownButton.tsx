...
import { useSubClasClientFiles } from "../../hooks/useSubClasClientFiles";
import Cookies from "js-cookie";
import {
  getClientSubclassifications,
  importClientSubclassifications,
} from "../../services/clientsSubclassificationSlice";
...
// Claim consts
const printClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Subclasificación de clientes.Imprimir";
...
const DropdownButton = () => {
  ...
  const clientSubclassification = useSelector(
    (state: RootState) =>
  ...
  const { handleExcel, handlePdf, handlePrint } = useSubClasClientFiles({
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${clientSubclassification?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    await dispatch(importClientSubclassifications({ token, file })).unwrap();
    ...
    <>
        ...
        <DropdownMenuContent>
          {hasClaim(printClaim) && (
            <DropdownMenuItem
              ...
              onClick={handlePrint}
              disabled={clientSubclassification?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-subclasificacion-clientes.xlsx"}
          formatFileName={"formato-subclasificacion-clientes.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
