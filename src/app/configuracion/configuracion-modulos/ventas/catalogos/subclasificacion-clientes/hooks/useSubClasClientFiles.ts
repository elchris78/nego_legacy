...
interface Props {
  clientSubclassification: ClientSubclassification[] | null;
}

export const useSubClasClientFiles = ({
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(clientSubclassification
        ? clientSubclassification.map((item) => {
            return [
    ...
    return {
      ...
      content: [
        { text: "Subclasificación de clientes", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("Subclasificación de clientes.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (clientSubclassification && clientSubclassification.length > 0) {
      const renamedData = clientSubclassification.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Sub clasificación": item.id,
        "Nombre de la Subclasificación": item.nombre,
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        "Subclasificación_de_clientes"
      );

      XLSX.writeFile(workbook, "Subclasificación_de_clientes.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Subclasificación de clientes",
    });
  };
