...
interface Props {
  subclassifications: ClientSubclassification[] | null;
}

export const generateSubclassificationFiles = ({ subclassifications = [] }: Props) => {
  ...
  const getDocDefinition = (
    subclassifications: ClientSubclassification[],
    forPrint: boolean
  ) => {
    ...
    content.push({
      text: 'Subclasificación de clientes',
      style: 'title',
    });
  ...
  const exportPdf = async (mode: "download" | "print") => {
    if (!pdfMake) return;
    try {
      const docDefinition = getDocDefinition(subclassifications || [], mode === 'print');
      await new Promise<void>((resolve) => {
        const pdf = pdfMake.createPdf(docDefinition);
        if (mode === "download") {
          const fileName = generateTimestampedFileName(
            "Subclasificación-clientes"
          );
  ...
  const handleExcel = async () => {
    ...
    const renamedData = subclassifications.map((item, index) => ({
      "UID Subclasificación": index+1,
      "ID Subclasificación": item.id,
