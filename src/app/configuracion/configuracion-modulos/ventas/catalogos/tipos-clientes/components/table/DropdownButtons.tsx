...
import { useClientTypesFiles } from "../../hooks/useClientTypesFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Tipos de clientes.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useClientTypesFiles({
    clientTypes,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${clientTypes?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      clientTypesActions.importClientTypesFromExcel({
        token,
        file,
      })
    );
    if (
      clientTypesActions.importClientTypesFromExcel.rejected.match(resultAction)
    ) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-tipo-clientes.xlsx"}
          formatFileName={"formato-tipo-clientes.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
