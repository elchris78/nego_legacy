...
const FormButtons = () => {
  ...
  const { handleSubmitForms, isFormComplete } = useClientTypesForm();
  ...
  const onAddClientType = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        clientTypesActions.createClientType({
          token,
          clientType: body,
        })
      );
      if (clientTypesActions.createClientType.rejected.match(resultAction)) {
      ...
      console.log("🚀 ~ onAddClientType ~ error:", error);
  ...
  const onEditClientType = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        clientTypesActions.updateClientType({
          token,
          id,
          clientType: body,
        })
      );
      if (clientTypesActions.updateClientType.rejected.match(resultAction)) {
        throw resultAction.payload;
      }
      ...
      console.log("🚀 ~ onEditClientType ~ error:", error);
    ...
    <div className="flex items-center justify-center mb-8 w-full">
      <div className="flex flex-col gap-5 sm:flex-row w-full justify-center">
        ...
        {mode === "view" && hasClaim(updateClaim) && (
          <Link
            href={`/configuracion/configuracion-modulos/ventas/catalogos/tipos-clientes/form?mode=edit&id=${id}`}
          >
