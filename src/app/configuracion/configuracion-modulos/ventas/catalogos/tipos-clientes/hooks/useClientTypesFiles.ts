...
export const useClientTypesFiles = ({ clientTypes = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(clientTypes || []).map((classification) => [
    ...
    return {
      ...
      content: [
        { text: "Tipos de clientes", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("tipos-clientes.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (clientTypes && clientTypes.length > 0) {
      const renamedData = clientTypes.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Tipo de cliente": item.id,
        Nombre: item.nombre,
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Tipos de clientes");

      XLSX.writeFile(workbook, "tipos-clientes.xlsx");
