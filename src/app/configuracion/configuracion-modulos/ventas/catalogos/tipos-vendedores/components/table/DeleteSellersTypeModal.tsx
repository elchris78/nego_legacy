...
const DeleteSellersTypeModal = ({ isOpenModal, onCloseModal, id }: Props) => {
  ...
  const handleDelete = async () => {
    try {
      const resultAction = await dispatch(
        sellersTypesActions.deleteSellersType({ token, id })
      );
      if (
        sellersTypesActions.deleteSellersType.rejected.match(
          resultAction
        )
      ) {
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar el Tipo de Vendedor de forma
            definitiva?
          </span>
        </div>
...
export default DeleteSellersTypeModal;
