...
import DeleteSellersTypeModal from "./DeleteSellersTypeModal";
...
const toggleStatusClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Tipos de vendedores.ToggleStatus";
const deleteClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Tipos de vendedores.Eliminar";
const updateClaim =
...
interface Props {
  index: number;
  startIndex: number;
  sellersType: SellersType;
}

const SellersTypesTableRow = ({ index, startIndex, sellersType }: Props) => {
  const token = Cookies.get("auth-token");
  const dispatch: AppDispatch = useDispatch();

  const [isActive, setisActive] = useState<boolean>(
    sellersType?.estatus ?? false
  );
  const [isOpenModal, setIsOpenModal] = useState(false);

  useEffect(() => {
    setisActive(sellersType?.estatus);
  }, [sellersType?.estatus]);

  const handleToggleStatus = async (newState: boolean) => {
    try {
      const resultAction = await dispatch(
        sellersTypesActions.toggleSellersTypeStatus({
          token,
          id: sellersType?.id,
        })
      );
      if (
        sellersTypesActions.toggleSellersTypeStatus.rejected.match(
          resultAction
        )
      ) {
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell className="text-center">{sellersType?.id}</TableCell>
        <TableCell className="text-center text-[#3C98CB]">
          <Link
            className="cursor-pointer hover:underline"
            href={`/configuracion/configuracion-modulos/ventas/catalogos/tipos-vendedores/form?mode=view&id=${sellersType?.id}`}
          >
            {sellersType?.nombre}
          </Link>
        </TableCell>
            ...
            <PopoverContent align="center" className="p-2 w-fit">
              <div className="flex flex-row gap-2">
                {hasClaim(updateClaim) && (
                  <Link
                    href={`/configuracion/configuracion-modulos/ventas/catalogos/tipos-vendedores/form?mode=edit&id=${sellersType?.id}`}
