...
import { useSellersTypesFiles } from "../../hooks/useSellersTypesFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Ventas.Catálogos.Tipos de vendedores.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useSellersTypesFiles({
    sellersTypes,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${sellersTypes?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      sellersTypesActions.importSellersTypesFromExcel({
        token,
        file,
      })
    );
    if (
      sellersTypesActions.importSellersTypesFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
        ...
        <DropdownMenuContent>
          {hasClaim(printClaim) && (
            <DropdownMenuItem
              ...
              onClick={handlePrint}
              disabled={sellersTypes?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-tipo-vendedor.xlsx"}
          formatFileName={"formato-tipo-vendedor.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
