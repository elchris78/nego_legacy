...
const FormButtons = () => {
  ...
  const { handleSubmitForms, isFormComplete } = useSellersTypesForm();
  ...
  const onAddSellersType = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        sellersTypesActions.createSellersType({
          token,
          sellersTypes: body,
        })
      );
      if (sellersTypesActions.createSellersType.rejected.match(resultAction)) {
      ...
      console.log("🚀 ~ onAddSellersType ~ error:", error);
  ...
  const onEditSellersType = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        sellersTypesActions.updateSellersType({
          token,
          id,
          sellersTypes: body,
        })
      );
      if (sellersTypesActions.updateSellersType.rejected.match(resultAction)) {
        throw resultAction.payload;
      }
    ...
    <div className="flex items-center justify-center mb-8 w-full">
      <div className="flex flex-col gap-5 sm:flex-row w-full justify-center">
        ...
        {mode === "view" && hasClaim(updateClaim) && (
          <Link
            href={`/configuracion/configuracion-modulos/ventas/catalogos/tipos-vendedores/form?mode=edit&id=${id}`}
          >
