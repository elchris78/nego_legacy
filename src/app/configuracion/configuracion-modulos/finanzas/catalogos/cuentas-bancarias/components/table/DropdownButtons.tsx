...
import { useCuentasBancariasFiles } from "../../hooks/useCuentasBancariasFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Finanzas.Catálogos.Cuentas bancarias.Imprimir";
...
const DropdownButtons = ({ getData }: Props) => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useCuentasBancariasFiles({
    cuentasBancarias,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${cuentasBancarias?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      cuentasBancariasActions.importCuentasBancariasFromExcel({
        token,
        file,
      })
    );
    if (
      cuentasBancariasActions.importCuentasBancariasFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-cuentas-bancarias.xlsx"}
          formatFileName={"formato-cuentas-bancarias.xlsx"}
          isLoading={isPending}
        />
      )}
    </>
