...
import DeleteModal from "./DeleteCXPModal";

import type { CXP } from "../../services/cxpsTypes";
import { useDispatch, useSelector } from "react-redux";
import { AppDispatch, RootState } from "@/lib/store/store";
import { toggleStatus } from "../../services/CXPSlice";
...
const CXPTableRow = ({
  ...
  // Claims const
  const updateClaim = "Configuración.Configuración de módulos.Finanzas.Catálogos.Tipos de documentos CXP.Actualizar";
  const deleteClaim = "Configuración.Configuración de módulos.Finanzas.Catálogos.Tipos de documentos CXP.Eliminar";
  const toggleStatusClaim = "Configuración.Configuración de módulos.Finanzas.Catálogos.Tipos de documentos CXP.ToggleStatus"
          ...
          {tipoDocumento.tipoDocumento.length > 20 ? (
            <ComicTooltip title={tipoDocumento?.tipoDocumento} placement="top-start">
              <Link
                ...
                href={`/configuracion/configuracion-modulos/finanzas/catalogos/tipos-documentos-cxp/form?mode=view&id=${tipoDocumento?.id}`}
              >
            ...
            <PopoverContent align="center" className="p-2 w-fit">
              <div className="flex flex-row gap-2">
                {hasClaim(updateClaim) && (
                  <Link
                    href={`/configuracion/configuracion-modulos/finanzas/catalogos/tipos-documentos-cxp/form?mode=edit&id=${tipoDocumento?.id}`}
