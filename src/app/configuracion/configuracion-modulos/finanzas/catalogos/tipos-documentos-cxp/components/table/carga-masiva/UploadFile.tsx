...
import { getCuentasPorPagar, importCuentasPorPagar } from "../../../services/CXPSlice";
...
const UploadFile = ({ onCloseModal }: Props) => {
  ...
  try {
    await dispatch(importCuentasPorPagar({ token, file })).unwrap();
    ...
    await dispatch(
      getCuentasPorPagar({ token, requestParams: { page: 1, size: 20 } })
    );
  }
};
  ...
  const handleDownloadFormat = async () => {
    ...
    downloadLink.href = "/formatos/formato-tipo-documentos-cxp.xlsx"; // URL del archivo
    downloadLink.download = "Formato-CuentasPorPagar.xlsx";
    downloadLink.click();
  }
