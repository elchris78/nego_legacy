...
export const useFiles = ({ tipoDocumento = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    return {
      ...
      content: [
        { text: "Tipos de documentos de Cuentas por pagar", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("TiposDocumentosCxP.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (tipoDocumento && tipoDocumento.length > 0) {
      const renamedData = tipoDocumento.map((item) => ({
        ...
        "Clave": item.id,
        "Tipo de Documento CxP": item.tipoDocumento,
        Estatus: item.estatus ? "Activo" : "Inactivo",
      }));

      const worksheet = XLSX.utils.json_to_sheet(renamedData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "TiposDocumentosCxP");

      XLSX.writeFile(workbook, "TiposDocumentosCxP.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Tipos de documentos de cuentas por pagar",
    });
  };
