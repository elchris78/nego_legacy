...
import { useTipoContratosBFiles } from "../../hooks/useTipoContratosBFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Finanzas.Catálogos.Tipos de contratos bancarios.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useTipoContratosBFiles({
    tiposContratosB,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${tiposContratosB?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      tiposContratosBActions.importTiposContratosBFromExcel({
        token,
        file,
      })
    );
    if (
      tiposContratosBActions.importTiposContratosBFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
        ...
        <DropdownMenuContent>
          {hasClaim(printClaim) && (
            <DropdownMenuItem
              ...
              onClick={handlePrint}
              disabled={tiposContratosB?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-Tipos-contratos-B.xlsx"}
          formatFileName={"formato-Tipos-contratos-B.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
