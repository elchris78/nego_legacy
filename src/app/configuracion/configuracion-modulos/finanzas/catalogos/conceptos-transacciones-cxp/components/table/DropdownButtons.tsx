...
import { useTransaccionesDXPFiles } from "../../hooks/useTransaccionesDXPFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Finanzas.Catálogos.Conceptos transacciones CXP.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useTransaccionesDXPFiles({
    transaccionesDXP,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${transaccionesDXP?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      transaccionesDXPActions.importTransaccionesDXPFromExcel({
        token,
        file,
      })
    );
    if (
      transaccionesDXPActions.importTransaccionesDXPFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
        ...
        <DropdownMenuContent>
          {hasClaim(printClaim) && (
            <DropdownMenuItem
              ...
              onClick={handlePrint}
              disabled={transaccionesDXP?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-ConceptosTransaccionesCxP.xlsx"}
          formatFileName={"formato-ConceptosTransaccionesCxP.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
