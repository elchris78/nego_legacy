...
interface Props {
  tipoDocumento: CXC[] | null;
}

export const useFiles = ({ tipoDocumento = [] }: Props) => {
  ...
  const getDocDefinition = () => {
      ...
      ...(tipoDocumento
        ? tipoDocumento.map((item) => {
            return [
              ...
              item.id || "-",
              item.origen,
              { text: item.tipoDocumento || "-", color: "#3C98CB" },
    ...
    return {
      ...
      content: [
        { text: "Tipos de documentos de Cuentas por cobrar", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("TiposDocumentosCxC.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (tipoDocumento && tipoDocumento.length > 0) {
      const renamedData = tipoDocumento.map((item) => ({
        // UID: item.uid,
        UID: item.id && item.id.includes("-") ? item.id.split("-")[0] : "-",
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave": item.id,
        "Tipo de Documento CxC": item.tipoDocumento,
        Estatus: item.estatus ? "Activo" : "Inactivo",
      }));

      const worksheet = XLSX.utils.json_to_sheet(renamedData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "TiposDocumentosCxC");

      XLSX.writeFile(workbook, "TiposDocumentosCxC.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Tipos de documentos de cuentas por cobrar",
    });
  };
