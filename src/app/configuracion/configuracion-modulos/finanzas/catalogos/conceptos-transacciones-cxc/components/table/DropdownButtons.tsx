...
import { useTransaccionesDXCFiles } from "../../hooks/useTransaccionesDXCFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Finanzas.Catálogos.Conceptos transacciones CXC.Imprimir";
...
const DropdownButtons = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useTransaccionesDXCFiles({
    transaccionesDXC,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${transaccionesDXC?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      transaccionesDXCActions.importTransaccionesDXCFromExcel({
        token,
        file,
      })
    );
    if (
      transaccionesDXCActions.importTransaccionesDXCFromExcel.rejected.match(
        resultAction
      )
    ) {
    ...
    <>
        ...
        <DropdownMenuContent>
          {hasClaim(printClaim) && (
            <DropdownMenuItem
              ...
              onClick={handlePrint}
              disabled={transaccionesDXC?.length === 0}
            >
              Imprimir
            </DropdownMenuItem>
          )}
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-ConceptosTransaccionesCxC.xlsx"}
          formatFileName={"formato-ConceptosTransaccionesCxC.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
