...
import { getTransaccionesDXC } from "../../services/transaccionesDXCActions";

const createClaim =
  "Configuración.Configuración de módulos.Finanzas.Catálogos.Conceptos transacciones CXC.Crear";
...
const TransaccionesDXCTableFilters = ({ searchParams, setSearchParams }: Props) => {
  ...
  useEffect(() => {
    const fetchTypeSeller = async () => {
      try {
        const token = Cookies.get("auth-token");
        const resp = await getTransaccionesDXC({
          token,
      ...
      >
            ...
            <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
              ...
              >
                <AccordionFilters
                  ...
                  transaccionesDXC={contrapartida}
        ...
        <div className="w-auto flex gap-2">
          {hasClaim(createClaim) && (
            <Link
              href={
                "/configuracion/configuracion-modulos/finanzas/catalogos/conceptos-transacciones-cxc/form?mode=new"
              }
...
export default TransaccionesDXCTableFilters;
