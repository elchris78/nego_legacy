...
import { deleteCancelConcepts } from "../../services/conceptosCancelAction";
...
const DeleteCancelConceptModal = ({ isOpenModal, onCloseModal, id, getConceptoCancelacion }: Props) => {
  ...
  const handleDeleteArea = async () => {
    setIsLoading(true);
    try {
      const resp = await deleteCancelConcepts({id, token});
      if(resp?.success)
      Swal.fire({
        title: "¡ÉXITO!",
        text: resp?.message || "El Concepto de cancelación ha sido eliminado exitosamente.", 
        icon: "success",
      ...
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar el Concepto de cancelación. Por favor, inténtalo de nuevo.",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar el Concepto de cancelación de forma definitiva?
          </span>
        </div>
...
export default DeleteCancelConceptModal;
