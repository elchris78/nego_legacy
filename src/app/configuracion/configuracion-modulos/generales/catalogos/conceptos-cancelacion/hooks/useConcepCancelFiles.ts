...
export const useConcepCancelFiles = ({ cancelConcepts = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(cancelConcepts
        ? cancelConcepts.map((item) => {
            return [
              item.id && item.id.includes("-") ? item.id.split("-")[0] : "-",
              item.id || "-",
              { text: item.concepto || "-", color: "#3C98CB" },
              item.afectaA || "-",
              item.motivoSat,
            ];
          })
    ...
    return {
      ...
      content: [
        { text: "Conceptos de cancelación", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("ConceptosCancelacion.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (cancelConcepts && cancelConcepts.length > 0) {
      const renamedData = cancelConcepts.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Concepto cancelación": item.id,
        "Concepto de cancelación": item.concepto,
        "Afecta a": item.afectaA,
        "Motivo SAT ID": item.motivoSatId,
        Estatus: item.estatus ? "Activo" : "Inactivo",
      }));

      const worksheet = XLSX.utils.json_to_sheet(renamedData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "ConceptosCancelacion");

      XLSX.writeFile(workbook, "ConceptosCancelacion.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Conceptos de cancelación",
    });
  };
