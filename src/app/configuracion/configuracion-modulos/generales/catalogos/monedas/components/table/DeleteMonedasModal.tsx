...
const DeleteMonedasModal = ({ isOpenModal, onCloseModal, id }: Props) => {
  ...
  const handleDelete = async () => {
    try {
      const resultAction = await dispatch(
        monedasActions.deleteMonedas({ token, id })
      );
      if (
        monedasActions.deleteMonedas.rejected.match(
          resultAction
        )
      ) {
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar esta Moneda?
          </span>
        </div>
...
export default DeleteMonedasModal;
