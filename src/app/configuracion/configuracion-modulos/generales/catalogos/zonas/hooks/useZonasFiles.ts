...
export const useZonasFiles = ({ zonas = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(zonas || []).map((zona) => [
        zona.id && zona.id.includes("-") ? zona.id.split("-")[0] : "-",
        zona.id || "-",
        { text: zona.nombre || "-", color: "#3C98CB" },
        zona.descripcion || "-",
        zona.estatus ? "Activo" : "Inactivo",
      ]),
    ];
    return {
      pageSize: "A4",
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("zonas.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (zonas && zonas.length > 0) {
      const renamedData = zonas.map((item) => ({
        UID: item.uidZona,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Zona": item.id,
        "Nombre de la zona": item.nombre,
