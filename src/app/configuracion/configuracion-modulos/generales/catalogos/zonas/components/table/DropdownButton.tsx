...
import { useZonasFiles } from "../../hooks/useZonasFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Generales.Catálogos.Zonas.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useZonasFiles({
    zonas,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${zonas?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      zonasActions.importZonas({
        token,
        file,
      })
    );
    if (zonasActions.importZonas.rejected.match(resultAction)) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-zonas.xlsx"}
          formatFileName={"formato-zonas.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
