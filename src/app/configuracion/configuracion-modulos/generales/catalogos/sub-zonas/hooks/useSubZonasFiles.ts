...
export const useSubZonasFiles = ({ subZonas = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      ...
      ...(subZonas || []).map((subZona) => [
        subZona.id && subZona.id.includes("-") ? subZona.id.split("-")[0] : "-",
        subZona.id || "-",
        subZona.zonaNombre || "-",
        { text: subZona.nombre || "-", color: "#3C98CB" },
        subZona.descripcion || "-",
        subZona.estatus ? "Activo" : "Inactivo",
      ]),
    ];
    return {
      pageSize: "A4",
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("subZonas.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (subZonas && subZonas.length > 0) {
      const renamedData = subZonas.map((item) => ({
        UID: item.uidSubZona,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave SubZona": item.id,
        Zona: item.zonaId,
        Nombre: item.nombre,
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "SubZonas");

      XLSX.writeFile(workbook, "subZonas.xlsx");
