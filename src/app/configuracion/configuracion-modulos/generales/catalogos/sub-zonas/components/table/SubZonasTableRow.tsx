...
import DeleteSubZonaModal from "./DeleteSubZonaModal";
...
const toggleStatusClaim =
  "Configuración.Configuración de módulos.Generales.Catálogos.SubZonas.ToggleStatus";
const deleteClaim =
  "Configuración.Configuración de módulos.Generales.Catálogos.SubZonas.Eliminar";
const updateClaim =
...
const SubZonasTableRow = ({ subZona, index, startIndex }: Props) => {
  const token = Cookies.get("auth-token");
  const dispatch: AppDispatch = useDispatch();

  const [isActive, setisActive] = useState<boolean>(subZona?.estatus ?? false);
  const [isOpenModal, setIsOpenModal] = useState(false);

  useEffect(() => {
    setisActive(subZona?.estatus);
  }, [subZona?.estatus]);
  ...
  const handleToggleStatus = async (newState: boolean) => {
    try {
      const resultAction = await dispatch(
        subZonasActions.toggleSubZonaStatus({
          token,
          id: subZona?.id,
        })
      );
      if (subZonasActions.toggleSubZonaStatus.rejected.match(resultAction)) {
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell className="text-center">{subZona?.id}</TableCell>
        <TableCell className="text-center">{subZona?.zonaNombre}</TableCell>
        <TableCell className="text-center text-[#3C98CB]">
          <Link
            className="cursor-pointer hover:underline"
            href={`/configuracion/configuracion-modulos/generales/catalogos/sub-zonas/form?mode=view&id=${subZona?.id}`}
          >
            {subZona?.nombre}
          </Link>
        </TableCell>
            ...
            <PopoverContent align="center" className="p-2 w-fit">
              <div className="flex flex-row gap-2">
                {hasClaim(updateClaim) && (
                  <Link
                    href={`/configuracion/configuracion-modulos/generales/catalogos/sub-zonas/form?mode=edit&id=${subZona?.id}`}
...
export default SubZonasTableRow;
