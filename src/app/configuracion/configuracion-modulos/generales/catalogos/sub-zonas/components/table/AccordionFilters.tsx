...
const AccordionFilters = ({
  ...
  useEffect(() => {
    // Si no hay zonas seleccionadas, se asigna un array vacío
    // Colocar el label compuesto por el valor y el nombre de la zona
    const selectedOptions = searchParams.zonaClave?.map((value) => {
      const option = zonasOptions.find((opt) => opt.value === value);
      return option ? { value: option.value, label: option.label } : null;
    });
    setSelectedValues(selectedOptions?.filter(Boolean) as Option[]);
  }, [searchParams.zonaClave, zonasOptions]);
          ...
          <AccordionContent className="px-3 py-2 overflow-visible h-[37vh]">
            <div className="relative overflow-visible">
              <MultipleSelector
                ...
                defaultOptions={zonasOptions}
                value={selectedValues}
                inputReadOnly={false}
                placeholder="Seleccionar zona"
                triggerSearchOnFocus
                onSearch={async (searchTerm) => {
                  return zonasOptions?.filter((option) =>
                    option.label
                      .toLowerCase()
                ...
                onChange={(values) => {
                  ...
                  setSearchParams((prevState) => ({
                    ...prevState,
                    zonaClave: values.map((item) => item.value),
                  }));
                }}
                emptyIndicator={
