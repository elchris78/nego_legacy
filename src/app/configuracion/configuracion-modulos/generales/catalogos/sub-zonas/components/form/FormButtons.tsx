...
const FormButtons = () => {
  ...
  const { handleSubmitForms, currentSubZona, isFormComplete } =
    useSubZonasForm();
  ...
  const onAddSubZona = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        subZonasActions.createSubZona({
          token,
          subzona: body,
        })
      );
      if (subZonasActions.createSubZona.rejected.match(resultAction)) {
      ...
      console.log("🚀 ~ onAddSubZona ~ error:", error);
  ...
  const onEditSubZona = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        subZonasActions.updateSubZona({
          token,
          id,
          subzona: body,
        })
      );
      if (subZonasActions.updateSubZona.rejected.match(resultAction)) {
        throw resultAction.payload;
      }
    ...
    <div className="flex items-center justify-center mb-8 w-full">
      <div className="flex flex-col gap-5 sm:flex-row w-full justify-center">
        ...
        {mode === "view" && hasClaim(updateClaim) && (
          <Link
            href={`/configuracion/configuracion-modulos/generales/catalogos/sub-zonas/form?mode=edit&id=${currentSubZona?.id}`}
          >
