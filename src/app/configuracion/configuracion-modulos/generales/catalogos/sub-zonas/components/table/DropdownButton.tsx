...
import { useSubZonasFiles } from "../../hooks/useSubZonasFiles";
...
const printClaim =
  "Configuración.Configuración de módulos.Generales.Catálogos.SubZonas.Imprimir";
...
const DropdownButton = () => {
  ...
  const { handleExcel, handlePdf, handlePrint } = useSubZonasFiles({
    subZonas,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${subZonas?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      subZonasActions.importSubZonas({
        token,
        file,
      })
    );
    if (subZonasActions.importSubZonas.rejected.match(resultAction)) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-sub-zonas.xlsx"}
          formatFileName={"formato-sub-zonas.xlsx"}
          isLoading={isLoading}
        />
      )}
    </>
