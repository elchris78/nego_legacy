...
export const useReturnConceptsFiles = ({ returnConcepts = [] }: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    return {
      ...
      content: [
        { text: "Conceptos de devolución", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("conceptos-devolución.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (returnConcepts && returnConcepts.length > 0) {
      const renamedData = returnConcepts.map((item) => ({
        UID: item.uidConcepto,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Concepto Devolución": item.id,
        "Concepto de devolución": item.concepto,
        "Afecta a": item.afectaA,
        Estatus: item.estatus ? "Activo" : "Inactivo",
        "Fecha creación": transformDate(item.fechaCreacion!),
      }));
      ...
      XLSX.writeFile(workbook, "conceptos-devolución.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Conceptos de devolución",
    });
  };
