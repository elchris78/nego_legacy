...
const DeleteReturnConceptModal = ({ isOpenModal, onCloseModal, id }: Props) => {
  ...
  const handleDeleteArea = async () => {
    try {
      const resultAction = await dispatch(
        returnConceptsActions.deleteReturnConcept({ token, id })
      );
      if (
        returnConceptsActions.deleteReturnConcept.rejected.match(resultAction)
      ) {
      ...
      console.error("Error al eliminar el concepto de devolución:", error);
      Swal.fire({
        title: "Error",
        text: error.message,
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar el Concepto de Devolución de forma
            definitiva?
          </span>
        </div>
...
export default DeleteReturnConceptModal;
