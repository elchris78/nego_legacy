...
export const useRestrictionConceptsFiles = ({
  restrictionConcepts = [],
}: Props) => {
  ...
  const getDocDefinition = () => {
    ...
    const tableBody = [
      tableHeaders,
      ...(restrictionConcepts || []).map((classification) => [
        classification.id && classification.id.includes("-")
          ? classification.id.split("-")[0]
          : "-",
        classification.id,
        { text: classification.concepto || "-", color: "#3C98CB" },
        classification.descripcion || "-",
        classification.advertencia || "-",
        classification.aplicaPara || "-",
        classification.requiereAutorizacion ? "Sí" : "No",
        classification.requiereNotificacion ? "Sí" : "No",
        classification.correosNotificacion?.length
          ? classification.correosNotificacion.join(", ")
          : "-",
    ...
    return {
      ...
      content: [
        { text: "Conceptos de restricciones", style: "header" },
        {
          style: "tableExample",
          table: {
            headerRows: 1,
  ...
  const exportPdf = async (mode: "download" | "print") => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        if (mode === "download") {
          pdf.download("conceptos-restricciones.pdf", () => resolve());
        } else {
          pdf.print();
          resolve();
        }
  ...
  const handleExcel = async () => {
    if (restrictionConcepts && restrictionConcepts.length > 0) {
      const renamedData = restrictionConcepts.map((item) => ({
        UID: item.uid,
        Prefijo: item.id && item.id.includes("-") ? item.id.split("-")[0] : "",
        "Clave Concepto de Restricciones": item.id,
        Concepto: item.concepto,
        Descripción: item.descripcion,
        Advertencia: item.advertencia,
        "Aplica para": item.aplicaPara,
        "Requiere autorización": item.requiereAutorizacion ? "Sí" : "No",
        "Clave de autorización": item.claveAutorizacion,
        "Requiere notificación": item.requiereNotificacion ? "Sí" : "No",
        "Correos notificación": item.correosNotificacion?.length
          ? item.correosNotificacion.join(", ")
          : "-",
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        "Conceptos de restricciones"
      );

      XLSX.writeFile(workbook, "conceptos-restricciones.xlsx");
  ...
  const setLogOnBackend = async (
    ...
    await registerUserActivity({
      ...
      subModule: "Conceptos de restricciones",
    });
  };
