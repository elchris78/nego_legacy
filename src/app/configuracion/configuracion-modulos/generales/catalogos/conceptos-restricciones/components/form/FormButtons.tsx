...
const FormButtons = () => {
  ...
  const { handleSubmitForms, currentRestrictionConcept, isFormComplete } =
  ...
  const onHandleSubmit = async () => {
    switch (mode) {
      case "new":
        onAddRestrictionConcept();
        break;
      case "edit":
        onEditRestrictionConcept();
        break;
      default:
        break;
    }
  ...
  const onAddRestrictionConcept = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        restrictionConceptsActions.createRestrictionConcept({
          token,
          data: body,
        })
      );
      if (
        restrictionConceptsActions.createRestrictionConcept.rejected.match(
          resultAction
        )
      ) {
      ...
      console.error("Error al agregar el concepto de restricción:", error);
  ...
  const onEditRestrictionConcept = async () => {
    if (!isFormComplete) return;
    try {
      const body = await handleSubmitForms();
      if (body === undefined) return;
      ...
      const resultAction = await dispatch(
        restrictionConceptsActions.updateRestrictionConcept({
          token,
          id,
          data: body,
        })
      ...
      if (
        restrictionConceptsActions.updateRestrictionConcept.rejected.match(
          resultAction
        )
      ) {
        throw resultAction.payload;
      ...
      console.log("🚀 ~ onEditRestrictionConcept ~ error:", error);
    ...
    <div className="flex items-center justify-center mb-8 w-full">
      <div className="flex flex-col gap-5 sm:flex-row w-full justify-center">
        ...
        {mode === "view" && hasClaim(updateClaim) && (
          <Link
            href={`/configuracion/configuracion-modulos/generales/catalogos/conceptos-restricciones/form?mode=edit&id=${id}`}
          >
