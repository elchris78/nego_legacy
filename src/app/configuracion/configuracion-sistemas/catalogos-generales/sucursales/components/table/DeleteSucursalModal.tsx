...
export const DeleteSucursalModal = ({
  isOpenModal,
  onCloseModal,
  id,
  getPlantillas,
  ...
  const handleDeleteSucursal = async () => {
    try {
      ...
      // toast(
      //   <ToastSuccessMsg description="Se ha eliminado la sucursal de forma exitosa" />
      // );
      onCloseModal();
    } catch (error) {
      console.log("🚀 ~ handleDeleteUser ~ error:", error);
      // toast(
      //   <ToastErrorMsg description="Ocurrió un error al eliminar la sucursal" />
      // );
    } finally {
      setIsLoading(false);
    }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl relative">
          <div className="bg-[#D05559] text-white px-6 py-4 rounded-t-xl flex justify-between items-center">
            <h2 className="text-3xl font-thin">Eliminar sucursal</h2>
            <button
          ...
          <div className="flex flex-row px-8 py-4">
            ...
            <div className="flex justify-center items-center text-center">
              <span className="antialiased text-sm font-semibold">
                ¿Estás seguro que deseas eliminar la sucursal de
                forma definitiva?
              </span>
            </div>
          </div>
