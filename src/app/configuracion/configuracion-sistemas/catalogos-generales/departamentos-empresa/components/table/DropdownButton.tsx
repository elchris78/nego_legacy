...
import { importDepartmentsFromExcelFile } from "@/lib/services/departments/departmentsSlice";
import { useDepartmentsFiles } from "../../hooks/useDepartmentsFiles";
...
const DropdownButton = ({ getData }: { getData: () => Promise<void> }) => {
  ...
  const { handlePdf, handleExcel, handlePrint } = useDepartmentsFiles({
    departments,
  });
  ...
  const handleDownload = (type: "pdf" | "excel") => {
    ...
    swalWithBootstrapButtons
      .fire({
        title: "¡Atención!",
        text: `Al dar aceptar se descargará el total de ${departments?.length} registros. Puede modificar en la parte inferior `,
  ...
  const handleFile = async (file: File) => {
    const resultAction = await dispatch(
      importDepartmentsFromExcelFile({ token, file })
    );

    if (importDepartmentsFromExcelFile.rejected.match(resultAction)) {
    ...
    <>
      ...
      {isUploadModalOpen && (
        <UploadDataModal
          ...
          getData={getData}
          urlFormatFile={"/formatos/formato-departamentos.xlsx"}
          formatFileName={"formato-departamentos.xlsx"}
          isLoading={isPending}
        />
      )}
    </>
