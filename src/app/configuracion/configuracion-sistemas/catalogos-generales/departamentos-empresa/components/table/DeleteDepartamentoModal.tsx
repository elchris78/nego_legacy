...
import { deleteDepartment } from "@/services/departments/departmentsSlice";
...
export const DeleteDepartamentoModal: React.FC<
  DeleteDepartamentoModalProps
> = ({ isOpenModal, onCloseModal, id, getDepartamentos }) => {
  ...
  const handleDeleteDepartamento = async () => {
    try {
      ...
      const response = await dispatch(
        deleteDepartment({ token, request: { departmentId: id } })
      ).unwrap();
      ...
      Swal.fire({
        title: "¡Error!",
        text: error.message || "Error al eliminar el departamento",
        icon: "error",
      ...
      <div className="flex flex-col gap-4 py-5 px-5 sm:px-16 lg:px-24">
        ...
        <div className="text-center text-[#5D6D7E]">
          ...
          <span className="text-xl font-light">
            ¿Estás seguro que deseas eliminar este departamento de forma
            definitiva?
          </span>
        </div>
