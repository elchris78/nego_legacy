...
const ColaboradoresAccordion = ({ field, idx }: Props) => {
  const { handleRemoveEmergencyContact } = useColaboradorFormContext();
    ...
    <div className="flex flex-col gap-6 w-full">
      <Accordion disableGutters sx={accordionStyles}>
        <AccordionSummary
          expandIcon={<Down />}
          sx={summaryStyles}
          aria-controls={`contact-${idx}-content`}
          id={`contact-${idx}-header`}
        >
        ...
        <AccordionDetails>
          <div className="flex flex-col gap-4">
            ...
            <ColaboradorFields key={field.id} field={field} idx={idx} />
          </div>
        </AccordionDetails>
      </Accordion>
    </div>
...
export default ColaboradoresAccordion;
