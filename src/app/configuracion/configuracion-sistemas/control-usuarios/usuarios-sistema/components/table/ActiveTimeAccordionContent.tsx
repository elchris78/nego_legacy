...
const ActiveTimeAccordionContent = ({
  searchParams,
  setSearchParams,
}: {
  ...
  const [timeFrom, setTimeFrom] = useState<Date | null>(
    searchParams.activePeriodStart
      ? parse(searchParams?.activePeriodStart, "HH:mm", new Date())
      : null
  );
  const [timeTo, setTimeTo] = useState<Date | null>(
    searchParams.activePeriodEnd
      ? parse(searchParams?.activePeriodEnd, "HH:mm", new Date())
      : null
  );

  const [time, setTime] = useState<Date | null>(() => {
    if (!searchParams.minActiveHours) return null;

    const decimalTime = parseFloat(searchParams.minActiveHours);
    ...
    switch (field) {
      case "timeRange":
        ...
        if (!checked) {
          ...
          if (
            searchParams.activePeriodStart !== undefined ||
            searchParams.activePeriodEnd !== undefined
          ) {
            setSearchParams({
              ...searchParams,
      ...
      case "time":
        ...
        if (!checked) {
          ...
          // Actualiza searchParams solo si es necesario (es decir, si alguno de los valores no es undefined)
          if (searchParams.minActiveHours !== undefined) {
            setSearchParams({
              ...searchParams,
  ...
  const handleTimeChange = (date: Date | null | undefined, field: string) => {
    if (field === "from") {
      ...
      if (date) {
        ...
        setSearchParams({ ...searchParams, activePeriodStart: timeString });
      } else {
        setSearchParams({ ...searchParams, activePeriodStart: undefined });
      }
    } else {
      setTimeTo(date!);
      ...
      if (date) {
        const timeString = format(date, "HH:mm");
        setSearchParams({ ...searchParams, activePeriodEnd: timeString });
      } else {
        setSearchParams({ ...searchParams, activePeriodEnd: undefined });
      }
    }
  };
                ...
                <TimePicker
                  ...
                  onChange={(date) => {
                    if (date) {
                      ...
                      setSearchParams({
                        ...searchParams,
                        minActiveHours: decimalTime.toFixed(2), // Redondea a 2 decimales
                      });
                    } else {
                      setSearchParams({
                        ...searchParams,
...
export default ActiveTimeAccordionContent;
