...
const LogoutAccordionContent = ({
  ...
  const [date, setDate] = useState<DateRange | undefined>({
    from: searchParams.logoutStartDate
      ? parse(searchParams?.logoutStartDate, "yyyy-MM-dd", new Date())
      : undefined,
    to: searchParams.logoutEndDate
      ? parse(searchParams?.logoutEndDate, "yyyy-MM-dd", new Date())
      : undefined,
  });
  ...
  const [timeFrom, setTimeFrom] = useState<Date | null>(
    searchParams.logoutStartTime
      ? parse(searchParams?.logoutStartTime, "HH:mm", new Date())
      : null
  );
  const [timeTo, setTimeTo] = useState<Date | null>(
    searchParams.logoutEndTime
      ? parse(searchParams?.logoutEndTime, "HH:mm", new Date())
      : null
  );
    ...
    switch (field) {
      case "date":
        ...
        if (!checked) {
          ...
          if (
            searchParams.logoutStartDate !== undefined ||
            searchParams.logoutEndDate !== undefined
          ) {
            setSearchParams({
              ...searchParams,
      ...
      case "time":
        ...
        if (!checked) {
          ...
          // Actualiza searchParams solo si es necesario (es decir, si alguno de los valores no es undefined)
          if (
            searchParams.logoutStartTime !== undefined ||
            searchParams.logoutEndTime !== undefined
          ) {
            setSearchParams({
              ...searchParams,
  ...
  const handleDateChange = (range: DateRange | undefined) => {
    ...
    if (newRange && newRange.from) {
      ...
      setSearchParams((prev) => ({
        ...prev,
        logoutStartDate: startDate,
        logoutEndDate: endDate,
      }));
    } else {
  ...
  const handleTimeChange = (date: Date | null | undefined, field: string) => {
    if (field === "from") {
      ...
      if (date) {
        ...
        setSearchParams({ ...searchParams, logoutStartTime: timeString });
      } else {
        setSearchParams({ ...searchParams, logoutStartTime: undefined });
      }
    } else {
      setTimeTo(date!);
      ...
      if (date) {
        const timeString = format(date, "HH:mm");
        setSearchParams({ ...searchParams, logoutEndTime: timeString });
      } else {
        setSearchParams({ ...searchParams, logoutEndTime: undefined });
      }
    }
  };
...
export default LogoutAccordionContent;
