...
import { fetchGetTemplates } from "../../services/usersActions";
...
export const SelectPlantillaModal = ({ onClose, isOpen, onSelectClaims }: Props) => {
  ...
  const getTemplates = async () => {
    try {
      setIsLoading(true);
      const response = await fetchGetTemplates({ token });
      // Accedemos a roleTemplates, ya que ese es el campo correcto
      const templates = response.roleTemplates || [];
      setTemplates(templates);
    } catch (error) {
      console.log("🚀 ~ getTemplates ~ error:", error);
      toast(
        <ToastErrorMsg description="Ocurrió un error al obtener las plantillas" />
      );
    } finally {
      setIsLoading(false);
    }
  ...
  // Fetch plantillas cuando el modal se abra
  useEffect(() => {
    if (isOpen) {
      const fetchPlantillas = async () => {
        ...
        setError(null);
        try {
          const response = await fetchGetTemplates({ token: token });
          // Accedemos a roleTemplates, ya que ese es el campo correcto
          const templates = response.roleTemplates || []; // Asegúrate de que roleTemplates exista
          // Mapear las plantillas a formato compatible con MultipleSelector
          const options = templates.map((template: any) => ({
            label: template.roleTemplateName, // Acceder a roleTemplateName
            value: template.roleTemplateId, // Acceder a roleTemplateId
          }));
