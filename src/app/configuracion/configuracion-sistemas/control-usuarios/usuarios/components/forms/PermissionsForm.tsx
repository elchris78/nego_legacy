...
import { SelectPlantillaModal } from "./SelectPlantillaModal"
...
interface ModuleSectionProps {
  module: string
  sections: Set<string> // Se mantiene para no afectar otras funcionalidades, aunque no se use en la representación
  subSections: Record<string, Set<string>>
...
const ModuleSection = ({ module, sections, subSections, selectedClaims, setSelectedClaims, permissionType }: ModuleSectionProps) => {
...
export const ClaimsManager = ({ claims, permissionType, handleDelete }: dataClaims) => {
  ...
  // console.log("ClaimsManager userClaims:", userClaims)
  useEffect(() => {
    if ((userClaims?.individualClaims?.length ?? 0) > 0) {
      const newSelectedClaims: SelectedClaims = {}
  
      userClaims?.individualClaims?.forEach(({ claimValue }) => {
        const parts = claimValue.split(".")
        const module = parts[0]
        const action = parts[parts.length - 1]
  ...
  }, [userClaims]); // Se ejecuta solo si los claims cambian
  ...
  useEffect(() => {
    ...
    console.log("Claims seleccionados:", assignClaims)
...
export const PermissionsForm: React.FC<Props> = ({ permissionType, handleDelete }) => {
  ...
  useEffect(() => {
    if (token && claimsData && claimsData.length === 0 && !isLoading) {
      dispatch(fetchAllClaims(token));
    }
  }, [token, claimsData, isLoading, dispatch]);
  ...
  return (
    <>
      <CardContent sx={{ p: 0 }}>
        ...
        <ClaimsManager claims={claimsData} permissionType={permissionType} handleDelete={handleDelete}/>
      </CardContent>
    </>
  )
}
