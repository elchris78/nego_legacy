...
export const DeleteUserModal: React.FC<DeleteUserModalProps> = ({ isOpenModal, onCloseModal, id }) => {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      toast(<ToastSuccessMsg description="Se ha eliminado el usuario de forma exitosa" />)
      onCloseModal()
      router.push("/admin/usuarios")
    } catch (error: any) {
      console.log("🚀 ~ handleDeleteUser ~ error:", error)
      const errorMsg = error?.message
      toast(<ToastErrorMsg description={errorMsg ?? "Ocurrió un error al eliminar el usuario"} />)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      <ContentBox>
        ...
        >
          <svg width="110" height="120" viewBox="0 0 170 170" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
          ...
          >
            Al eliminar el usuario, ya no podrá acceder a la plataforma, pero todas sus operaciones seguirán registradas
            en la base de datos.
          </Typography>
        </DialogContent>
        <DialogActions
