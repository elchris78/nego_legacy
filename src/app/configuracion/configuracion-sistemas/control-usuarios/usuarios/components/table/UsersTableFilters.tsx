...
export const UsersTableFilters = ({ searchParams, setSearchParams }: Props) => {
  const token = Cookies.get("auth-token") || "";

  const [inputValue, setInputValue] = useState(""); // Valor del input sin debounce
  const [isPopoverOpen, setIsPopoverOpen] = useState(false);

  const [roltemplatesOptions, setRolTemplatesOptions] = useState([]);
  ...
  useEffect(() => {
    if (token) {
      fetchAllRolTemplates();
    }
  }, []);

  // Fetch all role templates and asign them to the state
  const fetchAllRolTemplates = async () => {
    if (!token) return;
    const resp = await fetch(
      `${urlbase}NegoAuth/RoleTemplates/role-templates`,
      {
        method: "GET",
        headers: {
    ...
    setRolTemplatesOptions(
      data?.roleTemplates?.map(
        (company: { roleTemplateId: string; roleTemplateName: string }) => ({
          value: company.roleTemplateId,
          label: company.roleTemplateName,
        })
      )
    );
  };
    ...
    <div className="px-4">
      ...
      >
            ...
            <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
              ...
              >
                <AccordionFilters
                  ...
                  plantillasOptions={roltemplatesOptions}
        ...
        <div className="w-auto flex gap-2">
          {(hasClaim(
            "Configuración.Configuración del sistema.Usuarios.Usuario.Crear"
          ) ||
            hasClaim("Configuración.Usuarios.Crear")) && (
            <Link
              href={
                "/configuracion/configuracion-sistemas/control-usuarios/usuarios/formUser?mode=new"
              }
