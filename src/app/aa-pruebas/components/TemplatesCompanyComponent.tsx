...
} from "@/app/configuracion/configuracion-sistemas/control-usuarios/plantillas/services/plantillasCompanySlice";
...
const RoleTemplatesCompanyComponent: React.FC = () => {
  const dispatch: AppDispatch = useDispatch();
  const roleTemplatesList = useSelector((state: RootState) => state.plantillasCompany.roleTemplates);
  const totalRecordsInTemplatesList = useSelector((state: RootState) => state.plantillasCompany.totalRecordsTemplates)
  const roleTemplateByIdData = useSelector((state: RootState) => state.plantillasCompany.roleTemplateById);
  const loading = useSelector((state: RootState) => state.plantillasCompany.loading);
  const error = useSelector((state: RootState) => state.plantillasCompany.error);
  ...
  return (
    <div className="p-5 font-sans bg-gray-50">
      <h1 className="text-2xl font-bold text-blue-600">Gestión de Role Templates para usuarios no admin</h1>
      ...
      <div className="mt-6 bg-white p-4 shadow rounded">
        ...
        <div className="flex items-center mb-2">
          <label className="mr-2">Rol por defecto:</label>
          <input
            type="checkbox"
            checked={newTemplate.defaultRole}
            onChange={(e) => setNewTemplate({ ...newTemplate, defaultRole: e.target.checked })}
          />
        </div>
        {/* Inputs para Claims */}
        <div className="mb-2">
...
export default RoleTemplatesCompanyComponent;
