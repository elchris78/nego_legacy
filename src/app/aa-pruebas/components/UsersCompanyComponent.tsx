...
import {
  ...
  createUserNoClaims,
  assignClaims,
} from "../../configuracion/configuracion-sistemas/control-usuarios/usuarios/services/usersCompanySlice";

// Importa el type para ClaimDto
import { ClaimDto } from "@/app/admin/usuarios/services/adminUsersTypes";

//
// Subcomponente para ingresar Individual Claims
//
const UserClaimsForm: React.FC<{
  claims: ClaimDto[];
  ...
  const addClaim = () => {
    ...
    setClaims([...claims, { claimType: claimType.trim(), claimValue: claimValue.trim() }]);
    setClaimType("");
    setClaimValue("");
  };
  ...
  return (
    <div className="border p-2 rounded mt-2">
      <h4 className="text-sm font-semibold">Individual Claims</h4>
      <div className="flex gap-2 mt-1">
        ...
        <input
          ...
          className="border p-2 rounded flex-1"
        />
        <button onClick={addClaim} className="bg-purple-500 text-white px-2 rounded">
          Agregar
        </button>
      </div>
      {claims.length > 0 && (
...
const UsersCompanyComponent: React.FC = () => {
  const dispatch: AppDispatch = useDispatch();
  const currentUser = useSelector((state: RootState) => state.usersCompany.user);
  const usersList = useSelector((state: RootState) => state.usersCompany.users);
  const loading = useSelector((state: RootState) => state.usersCompany.loading);
  const error = useSelector((state: RootState) => state.usersCompany.error);
  const token = Cookies.get("auth-token") || "";

  useEffect(() => {
    console.log("UsersCompanyComponent mounted - loading:", loading);
  }, [loading]);

  // Estado para filtros en fetchUsers
  ...
  // Payload para crear usuario sin claims
  const [createNoClaimsPayload, setCreateNoClaimsPayload] = useState({
    ...
    userType: "",
    roleTemplateId: "",
    individualClaims: [] as ClaimDto[], // Se enviará como array vacío
  });

  // Payload para asignar claims
  ...
  // Campo para operaciones basadas en ID
  ...
  const handleCreateUser = () => {
    dispatch(createNewUser({ token, body: createPayload }));
  };
  ...
  const handleUpdateUser = () => {
    ...
    dispatch(updateUser({ token, body: updatePayload }));
  };
  ...
  const handleCreateUserNoClaims = () => {
    dispatch(createUserNoClaims({ token, body: createNoClaimsPayload }));
  };
    ...
    dispatch(
      assignClaims({
        ...
        body: {
          ...
          roleTemplateId: assignPayload.roletemplateId,
          individualClaims: assignPayload.individualClaims,
        },
      })
    );
  };
  ...
  return (
    <div className="p-5 font-sans bg-gray-50 relative">
      ...
      <h1 className="text-2xl font-bold text-blue-600">
        Prueba Endpoints Usuarios No Admin
      </h1>
      ...
      <div className="mt-4 border p-4 rounded">
        ...
        <div className="mb-2">
          <label className="block text-sm font-medium">Página actual</label>
          <input
            type="number"
            placeholder="Página"
      ...
      <div className="mt-4 border p-4 rounded">
        ...
        <input
          ...
          onChange={(e) => setCreatePayload({ ...createPayload, userName: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <input
          type="email"
        ...
        <input
          ...
          value={createPayload.userType}
          onChange={(e) => setCreatePayload({ ...createPayload, userType: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <input
          type="text"
          placeholder="Role Template ID"
          value={createPayload.roleTemplateId}
          onChange={(e) => setCreatePayload({ ...createPayload, roleTemplateId: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        ...
        <UserClaimsForm
          claims={createPayload.individualClaims}
          setClaims={(claims) =>
            setCreatePayload({ ...createPayload, individualClaims: claims })
          }
        />
      ...
      <div className="mt-4 border p-4 rounded">
        ...
        <input
          type="text"
          placeholder="User Type"
          value={createNoClaimsPayload.userType}
          onChange={(e) =>
            setCreateNoClaimsPayload({ ...createNoClaimsPayload, userType: e.target.value })
          }
          className="border p-2 rounded mr-2"
        />
        <input
          type="text"
          placeholder="Role Template ID"
          value={createNoClaimsPayload.roleTemplateId}
          onChange={(e) =>
            setCreateNoClaimsPayload({ ...createNoClaimsPayload, roleTemplateId: e.target.value })
          }
          className="border p-2 rounded mr-2"
        />
        <div className="flex items-center mb-2">
      ...
      <div className="mt-4 border p-4 rounded">
        ...
        <input
          type="text"
          placeholder="User ID"
          value={updatePayload.userId}
          onChange={(e) => setUpdatePayload({ ...updatePayload, userId: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <input
          type="text"
        ...
        <input
          type="text"
          placeholder="User Type"
          value={updatePayload.userType}
          onChange={(e) => setUpdatePayload({ ...updatePayload, userType: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <input
          type="text"
          placeholder="Role Template ID"
          value={updatePayload.roleTemplateId}
          onChange={(e) => setUpdatePayload({ ...updatePayload, roleTemplateId: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        {/* Agregamos checkbox para isActive en update */}
        <div className="flex items-center mb-2">
          <input
            type="checkbox"
            checked={updatePayload.isActive}
            onChange={(e) =>
              setUpdatePayload({ ...updatePayload, isActive: e.target.checked })
            }
            className="mr-2"
          />
          <label className="text-sm font-medium">Active</label>
        ...
        <UserClaimsForm
          claims={updatePayload.individualClaims}
          setClaims={(claims) =>
            setUpdatePayload({ ...updatePayload, individualClaims: claims })
          }
        />
      ...
      <div className="mt-4 border p-4 rounded">
        ...
        <input
          ...
          onChange={(e) => setAssignPayload({ ...assignPayload, userId: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <input
          type="text"
          placeholder="Role Template ID"
          value={assignPayload.roletemplateId}
          onChange={(e) => setAssignPayload({ ...assignPayload, roletemplateId: e.target.value })}
          className="border p-2 rounded mr-2"
        />
        <UserClaimsForm
          claims={assignPayload.individualClaims}
          setClaims={(claims) =>
            setAssignPayload({ ...assignPayload, individualClaims: claims })
          }
        />
...
export default UsersCompanyComponent;
