...
export default function DeleteTemplate({ onConfirm, profileId, open, setOpen, title, text }: DeleteUserModalProps) {
  const handleConfirm = () => {
    onConfirm()
    setOpen(false)
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[725px] p-0 bg-white">
        <DialogHeader className="bg-[#3C98CB] text-white p-6">
          <DialogTitle className="flex justify-between items-center text-4xl font-normal">
            {title}
          </DialogTitle>
        </DialogHeader>
        <div className="flex ">
          ...
          <div className="p-6  w-3/4">
            <div className="border-[2px] border-solid border-red-500 p-3">
              <p className="text-sm text-gray-600 font-bold text-center pl-12 pr-12">
                {text} {profileId}
              </p>
            </div>
